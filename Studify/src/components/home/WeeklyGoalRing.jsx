...
const WeeklyGoalRing = ({ percent = 0, currentMinutes = 0, goalMinutes = 300 }) => {
  ...
  const s = useMemo(() => getRingStyles(colors), [colors]);
...
const getRingStyles = (colors) => StyleSheet.create({
  card: {
  ...
  pct: { color: colors.text, fontSize: 20, fontWeight: '900' },
  hours: { color: colors.textMuted, fontSize: 10, fontWeight: '700', marginTop: 2 },
  info: { flex: 1, marginLeft: 14 },
  title: { color: colors.textMuted, fontSize: 12, fontWeight: '900', letterSpacing: 0.4 },
  sub: { color: colors.text, fontSize: 14, fontWeight: '800', marginTop: 4 },
  bar: {
    marginTop: 8,
    height: 6,
    width: '100%',
  ...
  meta: { color: colors.textMuted, fontSize: 11, fontWeight: '700', marginTop: 6 },
});
