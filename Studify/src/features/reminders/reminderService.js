...
import { getUserSettings, updateReminderSettings, getDueTopicsForUser } from '../subjects/subjectsDb';
...
/**
 * FASE 2.1: corpo do lembrete cita as revisões vencidas (FSRS).
 * Nunca quebra o agendamento: em qualquer erro volta ao texto padrão.
 */
export async function getDueBody(userId, fallbackBody) {
  try {
    const due = await getDueTopicsForUser(userId);
    if (due && due.length > 0) {
      const nomes = due.slice(0, 2).map((d) => d.topicoNome).join(', ');
      const resto = due.length > 2 ? ` (+${due.length - 2})` : '';
      const plural = due.length > 1 ? 'ões' : 'ão';
      return `⏰ Você tem ${due.length} revis${plural} vencendo hoje: ${nomes}${resto}`;
    }
  } catch {
...
async function scheduleDaily(hour, minute, body) {
  const api = notif();
  ...
  const finalBody = body || `Seu lembrete das ${hh}:${mm} — abra o Studify e mantenha o streak 🔥`;
...
export async function enableDailyReminder(userId, hour, minute) {
  ...
  const hh = String(hour).padStart(2, '0');
  const mm = String(minute).padStart(2, '0');
  const body = await getDueBody(userId, `Seu lembrete das ${hh}:${mm} — abra o Studify e mantenha o streak 🔥`);
  const identifier = await scheduleDaily(hour, minute, body);
...
export async function restoreDailyReminder(userId) {
  try {
    ...
    if (settings.reminder_enabled) {
      await configureReminderChannel();
      const hour = settings.reminder_hour ?? 20;
      const minute = settings.reminder_minute ?? 0;
      const body = await getDueBody(userId, `Seu lembrete — abra o Studify e mantenha o streak 🔥`);
      await scheduleDaily(hour, minute, body);
    }
  } catch (e) {
